...
Test(glist_popfront, one_element)
{
    glist_t *list = glist_new(sizeof(int));
    int val = 14352;
    glist_pushback(list, &val);
    gnode_t *node = glist_popfront(list);
...
Test(glist_popfront, entire_list)
{
    glist_t *list = glist_new(sizeof(int));
...
Test(glist_popfront, too_far)
{
    ...
    while (glist_size(list)) {
        cr_expect_(glist_popfront(list) != NULL);
    }
    cr_expect_(glist_popfront(list) == NULL);
    cr_expect_(glist_popfront(list) == NULL);
    glist_destroy(list, NULL);
